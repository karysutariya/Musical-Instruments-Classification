...
def NeverSeenbinary2categorical(targetsIndex):
    # Input is 20 dimensional. Output is 20x2 dimensional
    categorical = np.zeros(20)
    categorical[targetsIndex] = 1
...
class NeverSeen_MICDataset(Dataset):
    # Pytorch dataset for Never seen
    def __init__(self, dataPath, missing=False):
    ...
    def __getitem__(self, index):
        X = self.X[index]
        # Y = binarize_targets(self.Y_true[index])
        Y = NeverSeenbinary2categorical(self.Y_true[index])
        X = torch.tensor(X, requires_grad=False, dtype=torch.float32)
